...
import { 
  Check, X, MessageSquare, Utensils, Star, 
  TrendingDown, TrendingUp, Clock, User, Filter 
} from 'lucide-react';
...
const WardenMessActivity = () => {
    ...
    const [filter, setFilter] = useState('All'); // All, Pending, Negative
    ...
    const handleUpdateStatus = async (activityId, newStatus) => {
        try {
            ...
            fetchAllActivities(); // Refresh data
        } catch (err) {
    ...
    if (loading) return <div className="p-10 font-black animate-pulse">LOADING WARDEN PANEL...</div>;
                                ...
                                <div key={act._id} className="group bg-slate-50 hover:bg-white hover:shadow-md transition-all p-5 rounded-3xl border border-transparent hover:border-orange-200 flex justify-between items-center">
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="bg-orange-500 size-2 rounded-full animate-pulse"></span>
                                            <p className="text-orange-600 font-black text-sm uppercase">{act.specialRequest.item}</p>
                                        </div>
                                    </div>
                                    
                                    <div className="flex gap-2">
                                        {act.specialRequest.status === 'Pending' ? (
                                            <>
                                                ...
                                                <button onClick={() => handleUpdateStatus(act._id, 'Rejected')} className="bg-slate-200 text-slate-500 p-3 rounded-2xl hover:bg-red-500 hover:text-white transition-all">
                                                    <X size={20}/>
                                                </button>
                                            </>
                                        ) : (
                                            <div className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase ${act.specialRequest.status === 'Approved' ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'}`}>
                                                {act.specialRequest.status}
                                            </div>
                                        )}
                                    </div>
                                </div>
...
// Render section for requests in Warden Dashboard
{activities.filter(a => a.specialRequest?.item).map(act => {
    const isToday = act.date === new Date().toLocaleDateString('en-CA');
    
    return (
        <div key={act._id} className="p-4 border-b">
            <p className="font-bold">{act.student?.name} - {act.specialRequest.item}</p>
            
            {/* Show buttons ONLY if it's today's request and pending */}
            {isToday && act.specialRequest.status === 'Pending' ? (
                <div className="flex gap-2 mt-2">
                    <button onClick={() => handleUpdateStatus(act._id, 'Approved')} className="bg-green-500 text-white px-3 py-1 rounded">Approve</button>
                    <button onClick={() => handleUpdateStatus(act._id, 'Rejected')} className="bg-red-500 text-white px-3 py-1 rounded">Reject</button>
                </div>
            ) : (
