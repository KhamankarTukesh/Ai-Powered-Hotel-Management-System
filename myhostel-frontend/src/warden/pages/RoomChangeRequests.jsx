...
const WardenRoomRequests = () => {
  ...
  const [wardenNotes, setWardenNotes] = useState({}); // Track notes for each ID
  ...
  const handleProcess = async (requestId, action) => {
    ...
    try {
      ...
      // Remove from UI immediately as it is now processed
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="grid gap-6">
            {requests.length > 0 ? (
              requests.map((req) => (
                <div key={req._id} className="bg-white rounded-[2.5rem] p-8 shadow-xl border border-orange-50 flex flex-col gap-6">
                  
                  {/* Student & Room Info */}
                  <div className="flex flex-col md:flex-row gap-6 justify-between items-center">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-orange-50 rounded-2xl flex items-center justify-center text-orange-500 border border-orange-100">
                        <ArrowLeftRight size={24} />
                      </div>
                      <div>
                        <h3 className="text-md font-black text-slate-800 uppercase leading-none">{req.student?.fullName}</h3>
                        <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">{req.student?.studentDetails?.rollNumber}</p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-2 text-[10px] font-black text-slate-400 uppercase ml-2">
                    ...
                    <div className="relative">
                      ...
                      <textarea 
                        ...
                        className="w-full pl-12 pr-4 py-4 bg-slate-50 rounded-[1.5rem] text-xs font-bold outline-none focus:ring-2 ring-orange-100 min-h-[100px] border border-slate-100"
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button
                      ...
                      className="flex-1 bg-slate-900 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-green-600 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      disabled={actionLoading === req._id}
                      className="flex-1 bg-white border-2 border-slate-100 text-slate-400 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-red-50 hover:text-red-500 hover:border-red-100 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                    >
